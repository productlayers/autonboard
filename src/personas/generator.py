...
class PersonaGenerator:
    """Generates personas and infers high value actions for a given product."""
    
    def __init__(self):
        # Supports standard OPENAI_API_KEY and custom OPENAI_BASE_URL via env
    ...
    async def analyze_product(self, product_name: str, product_description: str, pm_hva: str) -> ProductAnalysis:
        """
        Takes a product name, description, and the PM's hypothesized HVA,
        and uses the LLM to generate a structural analysis including an audit of the HVA.
        """
        ...
        response = await self.client.beta.chat.completions.parse(
            model=self.model,
            messages=[
                {"role": "system", "content": "You are a UX researcher that outputs valid JSON matching the schema."},
