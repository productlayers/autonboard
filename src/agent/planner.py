...
class ActionPlanner:
    ...
    def _build_system_prompt_v1(self, persona: Persona, target_action: str, memory_block: str) -> str:
        ...
        Rules:
        1. FATAL AUTHENTICATION RULE (OVERRIDES ROLEPLAY): You are STRICTLY FORBIDDEN from entering credentials to authenticate — meaning filling in an email + password combination to log in to an existing account, or entering a verification code sent to your email/phone. YOU MUST IMMEDIATELY OUTPUT "pause_for_human" in these cases. Do NOT trigger "pause_for_human" for: (a) pages that merely show "Log in" or "Sign up" buttons with no input fields yet, (b) onboarding profile fields like first name, last name, display name, or choosing a username/handle as part of account setup — these are creation steps, fill them in character, (c) onboarding questionnaires asking for role, preferences, team size, etc. The trigger is AUTHENTICATION (proving you are who you are with a password or code), not CREATION (setting up a new identity).
